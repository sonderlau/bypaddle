    ...
    def answer_question(self, query: str, return_context: bool = False, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, result in enumerate(search_results):
                ...
                #                   f"开头100字符: {content[:100]}...\n"
                #                   f"结尾100字符: ...{content[-100:]}")
...
def call_qwen(answer: str, truth: str):
    
    
    messages = [
        {'role': 'system', 'content': '我将提供给你两个组数据，分别是我对手册内容的作答和手册中的真实内容，以 Markdown 格式保存的表格。请你对比这两组内容，特别注意对比其中的数字和整体结构，部分内容可能会换行显示，请你联系好上下行之间的内容，综合考虑逻辑的完整度和数字的准确，以及整个表格表达的意义。等级分为5个档次（不及格，及格，中等，良好，优秀），用于衡量作答的好坏。结果只返回给我一个评价的最终等级。'},
        {'role': 'user', 'content': f"这是我的作答内容: {answer} \n 这是文档中的实际记录的内容: {truth}"}]
...
def main():
    ...
    # 测试问题
    test_queries = None
    with open("test/test_tables/tables.json", "r", encoding="utf-8") as f:
    ...
    for query in test_queries:
        ...
        try:
            ...
            result = rag.answer_question(query["question"], return_context=True)
            

            
                 
            with open(f"test/test_tables/tables/{query['table_source']}.md", "r", encoding="utf-8") as f:
                
                table_truth = f.read()
                
                print(table_truth)
                resp = call_qwen(answer=result["answer"], truth=table_truth)
                test_results_comparison.append({
                    "question": query["question"],
                    "judgement": resp["output"]["text"],
                    "table_file": query['table_source'],
        ...
        with open("./output/test/test_tables_answers.json", "w", encoding="utf-8") as f:
            json.dump(test_results, f, ensure_ascii=False)
        
        with open("./output/test/test_tables_judgement.json", "w", encoding="utf-8") as f:
            json.dump(test_results_comparison, f, ensure_ascii=False)
